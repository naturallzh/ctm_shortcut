        ...
        <div class="mem-list">
          <div class="mem-detail" v-for="(mem, idx) of group">
            ...
            <div class="item-box" v-for="(buffItem, buffIdx) of buffListRes">
              ...
              <div v-else>?</div>
              <el-popconfirm v-if="mem.conflict[buffIdx] && !mem.provide.includes(buffIdx)" :title="(mem.conflict[buffIdx]?.length ? `该buff与${buffList[mem.conflict[buffIdx][0]].name}冲突，` : '') + '是否开启?'" width="200px" @confirm="confirmOpen(mem, buffIdx)">
                <template #reference>
...
const buffList = [
  { name: '嗜血 / 英勇', desc: '' }, { name: '5%全属性', desc: '' }, { name: '3%全伤害', desc: '' }, { name: '5%暴击', desc: '元素萨德天赋元素之誓按描述只提供法术暴击' },
  { name: '力量敏捷', desc: '' }, { name: '4%物理易伤', desc: '' }, { name: '攻强', desc: '近战20% 远程10%' }, { name: '12%破甲', desc: '' },
  { name: '10%攻速', desc: '' }, { name: '30%流血伤害', desc: '' }, { name: '10%法强', desc: '法师和非元素德萨满只能提供6%' }, { name: '5%法术急速', desc: '' },
  { name: '8%法术易伤', desc: '' }, { name: '5%法术易爆', desc: '' }, { name: '耐力', desc: '' }, { name: '护甲', desc: '' },
...
const classList = [
  ...
  { id: 'hunt-1', name: '兽王猎', role: 'd1', require: [0,1,2,3,4,5,6,7,8,14,15,16], passive: [2], conflict: [], desc: '', count: 0 },
  { id: 'hunt-2', name: '射击猎', role: 'd1', require: [0,1,2,3,4,5,6,7,8,14,15,16], passive: [6], conflict: [], desc: '', count: 0 },
  { id: 'hunt-3', name: '生存猎', role: 'd1', require: [0,1,2,3,4,5,6,7,8,14,15,16], passive: [8], conflict: [], desc: '', count: 0 },
